...
class MainActivity : AppCompatActivity() {
                    ...
                    if (arrayOf("1", "2", "3", "4", "5", "6", "7", "8", "9", "0").contains(child.text.toString()))
                        ...
                        child.setOnClickListener {
                            ...
                            when (command) {
                                ...
                                "." -> addSeparator(numTextBox)
                                "√" -> {
                                    if (!(operators.contains(lastChr.toString())))
    ...
    fun clearErr()
    {
        if (findViewById<TextView>(R.id.num_text).text.toString() == "Ошибка выражения" || findViewById<TextView>(R.id.num_text).text.toString() == "")
    ...
    {
        val numTextBox = findViewById<TextView>(R.id.num_text)
        findViewById<TextView>(R.id.calc_text).text = numTextBox.text.toString().plus("=")
        val result: String = eval(findViewById<TextView>(R.id.num_text).text.toString())
        numTextBox.text = result
    }

    fun addSeparator(numTextBox: TextView)
    {
        val regexString: String = operators.joinToString("|") { Regex.escape(it) }
    ...
    fun unaryMP(numTextBox: TextView)
    {
        ...
        if (isDouble(expression[expression.lastIndex]))
        {
            ...
            try
            {
                if (numTextBox.text[numTextBox.text.lastIndexOf(expression[expression.lastIndex]) - 1] == '-')
                    unChr = ' '
            }
            catch (e: Exception)
            {
